...
import { Dumbbell, History, User, LogOut, LayoutDashboard, Menu } from 'lucide-react';
...
function NavLink({ to, icon, text, onClick }: NavLinkProps) {
  return (
    <Link
      to={to}
      onClick={onClick}
      className="flex items-center space-x-1 px-3 py-2 rounded text-gray-600 hover:text-gray-900"
    >
      {icon}
      <span>{text}</span>
    </Link>
